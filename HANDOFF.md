...
- **Live site:** https://sdgmart.onrender.com (hosted on Render, auto-deploys on every `git push` to `main`)
...
1. **Run `supabase-schema-referrals.sql`** in Supabase (required for §8 to work).
2. **Swap Paystack to live keys** + set webhook + confirm account activated.
3. **Verify post-deploy:** place a test order → should appear in admin Orders with an `SDG-` code + PAID/COLLECT badge; "Track" should work; back button should not exit; admin refresh should not blank.
4. **Clarify & implement "admin dashboard items clickable to their pages"** — the user asked for this but the target was ambiguous (Dashboard top-products list? KPI cards? inventory rows?). Ask which items and where they should link.
5. **Deferred backlog** (see memory `project_sdgmart_deferred.md`): WhatsApp Cloud API store-bot; **recurring-orders cron** (data + UI exist, but NO job actually places due orders — `recurring_orders.next_run_at`); more push-subscribe prompts; saved cart/wishlist; phone/SMS login; neighborhood social proof; 2FA admin; audit log; Sentry (SENTRY_DSN hook ready); Dagbani/Twi i18n.
...
---
*Last updated at the end of the "big fix batch" commit `e452ef6`. Bump this doc as things change.*
