...
## ⭐ LATEST STATE — resume here (updated 2026-07-18)
- **Order-level reviews (2026-07-18):** the post-delivery prompt now asks ONE "Rate order SDG-xxxxx" per delivered order (items summarized underneath) instead of one prompt per product. `db.reviews.pendingForUser` returns unreviewed ORDERS (max 3); `createForOrder` stores product_id NULL; POST /api/me/reviews accepts orderId-only (ownership-checked; legacy productId path kept). **Requires `supabase-schema-order-reviews.sql` (§4.10) — NOT YET RUN.** Note: per-product star ratings were never displayed anywhere in the UI, so nothing visible was lost.
...
> §1–§13 below are still accurate; this block is the current front line. `sw.js` CACHE_NAME = `sdgmart-v65-route-batching-bulk-assign`.
