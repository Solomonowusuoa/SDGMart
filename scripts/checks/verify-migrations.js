...
const CHECKS = {
  ...
  'supabase-schema-advisors.sql': [['rls on schema_migrations', () => rls('schema_migrations')], ['rls on stock_holds', () => rls('stock_holds')], ['rls on order_events', () => rls('order_events')], ['every public function pins search_path', () => ask("(select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prokind='f' and p.proconfig is null)=0")], ['idx email_tokens_user_id_idx', () => idx('email_tokens_user_id_idx')], ['idx referrals_referrer_id_idx', () => idx('referrals_referrer_id_idx')], ['idx stock_holds_product_id_idx', () => idx('stock_holds_product_id_idx')], ['idx users_referred_by_idx', () => idx('users_referred_by_idx')]],
