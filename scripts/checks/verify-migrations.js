...
const CHECKS = {
  ...
  'supabase-schema-advisors.sql': [['rls on schema_migrations', () => rls('schema_migrations')], ['rls on stock_holds', () => rls('stock_holds')], ['rls on order_events', () => rls('order_events')], ['every public function pins search_path', () => ask("(select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prokind='f' and p.proconfig is null)=0")], ['idx email_tokens_user_id_idx', () => idx('email_tokens_user_id_idx')], ['idx referrals_referee_id_idx', () => idx('referrals_referee_id_idx')], ['idx users_referred_by_idx', () => idx('users_referred_by_idx')]],
  'supabase-schema-advisors-2.sql': [['referrals_referrer_id_idx dropped', () => ask("not exists(select 1 from pg_indexes where indexname='referrals_referrer_id_idx')")], ['reviews_user_id_idx dropped', () => ask("not exists(select 1 from pg_indexes where indexname='reviews_user_id_idx')")], ['stock_holds_product_id_idx dropped', () => ask("not exists(select 1 from pg_indexes where indexname='stock_holds_product_id_idx')")], ['no two indexes on a table share a column list', () => ask("(select count(*) from (select ci.relname nm, ct.relname tb, array_to_string(array(select pg_get_indexdef(x.indexrelid,k+1,true) from generate_series(0,x.indnatts-1) k), ',') cs, x.indpred is not null pt from pg_index x join pg_class ci on ci.oid=x.indexrelid join pg_class ct on ct.oid=x.indrelid join pg_namespace n on n.oid=ct.relnamespace where n.nspname='public') a join (select ci.relname nm, ct.relname tb, array_to_string(array(select pg_get_indexdef(x.indexrelid,k+1,true) from generate_series(0,x.indnatts-1) k), ',') cs, x.indpred is not null pt from pg_index x join pg_class ci on ci.oid=x.indexrelid join pg_class ct on ct.oid=x.indrelid join pg_namespace n on n.oid=ct.relnamespace where n.nspname='public') b on a.tb=b.tb and a.cs=b.cs and a.nm<b.nm and a.pt=b.pt)=0")]],
