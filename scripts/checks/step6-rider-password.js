...
async function cleanup(verbose = true) {
  for (const [tbl, ut] of [['users', 'user'], ['riders', 'rider']]) {
    ...
    for (const x of (rows.data || [])) {
      ...
      await sb.from('email_tokens').delete().eq(ut === 'rider' ? 'rider_id' : 'user_id', x.id).then(() => {}, () => {});
...
(async () => {
  ...
  try {
    ...
    // rider_id, not user_id: a rider token is owned by its own column now, and
    // querying the customer column is how an earlier version of this test
    // concluded "riders still cannot recover" when they perfectly well could.
    const tokens = await sb.from('email_tokens').select('token, purpose, user_id, rider_id').eq('rider_id', riderId);
    ...
    rec('R-PW', 'the token is owned by rider_id and NOT user_id',
      !!riderTok && riderTok.rider_id === riderId && riderTok.user_id === null,
      'rider_id = ' + (riderTok && riderTok.rider_id) + ' · user_id = ' + JSON.stringify(riderTok && riderTok.user_id));

    // ── The id-collision case: A-01, reopened and closed ──────────────────
    console.log('\n⭐ The id-collision case (A-01): a rider token must never touch a customer');
    // Built from scratch rather than by moving ids around: create a customer,
    // then a SECOND rider carrying that customer's exact id, so the two really
    // do collide. Moving an existing rider is not possible any more, because
    // email_tokens.rider_id now references riders — which is the cascade doing
    // its job.
    const clashCust = await sb.from('users').insert({
      name: 'RPW Customer', email: cEmail,
      password_hash: await db.hashPassword(OLD), role: 'customer', email_verified: true,
    }).select().single();
    custId = clashCust.data && clashCust.data.id;
    const twinEmail = `sdgtest-rpw-twin-${stamp}@example.com`;
    const twin = await sb.from('riders').insert({
      id: custId, name: 'RPW Twin Rider', email: twinEmail,
      password_hash: await db.hashPassword(OLD), phone: '0200000000',
    }).select().single();

    if (twin.error) {
      rec('A-01', 'a rider and a customer could be put on the same id for this test', null,
        'skipped — ' + twin.error.message.slice(0, 80));
    } else {
      console.log('  customer ' + custId + ' and rider ' + twin.data.id + ' now share one id — the A-01 condition\n');
      await call('POST', '/api/auth/forgot-password', { email: twinEmail });
      const tt = await sb.from('email_tokens').select('token, purpose, rider_id, user_id').eq('rider_id', custId);
      const twinTok = (tt.data || [])[0];
      rec('A-01', 'the twin rider gets a rider-scoped token', !!twinTok && twinTok.purpose === 'reset-rider',
        twinTok ? 'purpose ' + twinTok.purpose + ' · rider_id ' + twinTok.rider_id + ' · user_id ' + JSON.stringify(twinTok.user_id) : 'no token');

      const custBefore = (await sb.from('users').select('password_hash').eq('id', custId).single()).data.password_hash;
      const riderBefore = (await sb.from('riders').select('password_hash').eq('id', custId).single()).data.password_hash;
      const used = await call('POST', '/api/auth/reset-password', { token: twinTok.token, newPassword: 'Reset!Passw0rd42' });
      const custAfter = (await sb.from('users').select('password_hash').eq('id', custId).single()).data.password_hash;
      const riderAfter = (await sb.from('riders').select('password_hash').eq('id', custId).single()).data.password_hash;
      ...
      rec('A-01', 'the rider can sign in with the reset password',
        (await call('POST', '/api/auth/login', { email: twinEmail, password: 'Reset!Passw0rd42' })).status === 200, 'signed in');
      ...
      rec('A-01', 'the reset token is consumed and cannot be replayed',
        (await call('POST', '/api/auth/reset-password', { token: twinTok.token, newPassword: 'Another!Pass42' })).status >= 400,
        'replay refused');
      await sb.from('email_tokens').delete().eq('rider_id', custId);
      await sb.from('sessions').delete().eq('user_id', custId);
      await sb.from('riders').delete().eq('id', custId);
    }
  } finally {
    console.log('\nCleanup');
    await cleanup();
